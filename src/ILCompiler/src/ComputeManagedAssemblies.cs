    ...
    public class ComputeManagedAssemblies : Task
    {
        ...
        public override bool Execute()
        {
            ...
            for (int i = 0; i < assemblies.Length; i++)
            {
                ...
                // Skip the native apphost (whose name ends up colliding with the CoreRT output binary
                if (taskItem.ItemSpec.EndsWith("apphost.exe", StringComparison.OrdinalIgnoreCase))
                {
                    assembliesToSkipPublish.Add(taskItem);
                    continue;
                }
                ...
                try
                {
                    ...
                    var module = typeSystemContext.GetModuleFromPath(taskItem.ItemSpec);
                    if (module is EcmaAssembly)
                    {
                        string culture = module.MetadataReader.GetString(((EcmaAssembly)module).AssemblyDefinition.Culture);
